...
def test_create_category_objects_from_data(categories_list: list) -> None:
    ...
    assert (
        category_objects_list[0].description
        == "Смартфоны, как средство не только коммуникации"
    )
    assert (
        category_objects_list[1].description
        == "Современный телевизор, который позволяет наслаждаться просмотром"
    )
    assert category_objects_list[1].name == "Телевизоры"
    # Test products objects
    assert category_objects_list[0].products[0].name == "Samsung Galaxy C23 Ultra"
    assert category_objects_list[0].products[1].quantity == 8
    assert category_objects_list[1].products[0].name == '55" QLED 4K'
